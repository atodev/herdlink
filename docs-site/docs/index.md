...
HerdLink is a demonstration of a next-generation cattle collar concept: **direct-to-cell
satellite connectivity (Starlink NTN) instead of per-farm radio towers**, paired with
**herd-relative behavioural analytics** that flag unwell animals from collar telemetry alone.
...
| [Why direct-to-cell](./comparison) | The architecture and cost case against tower-based collars |

## The two claims

1. **Cheaper hardware architecture.** Tower-based systems carry per-farm infrastructure:
   a base station, a site survey, an install crew, and the capex recovery baked into the
   subscription. A direct-to-cell collar removes all of it — see
   [the comparison](./comparison).
2. **Smarter software.** Scoring every cow against the herd (rather than against fixed
   thresholds) makes detection robust to weather, season, and feed changes — the whole
   herd shifts together, the sick individual still stands out. The
   [detection page](./detection) explains the method; the demo lets you verify it against
